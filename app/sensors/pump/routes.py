...
def _clean_watchdog():
    ...
    while not _clean_stop.wait(config.CLEANING_TICK_SECONDS):
        try:
            reason = _cleaning_tick_reason()
        except Exception:
...
def _ensure_watchdog():
    ...
    with _clean_thread_lock:
        if _clean_thread is not None and _clean_thread.is_alive():
            return
        _clean_stop.clear()
        _clean_thread = threading.Thread(
            target=_clean_watchdog, name="cleaning-watchdog", daemon=True
        )
        _clean_thread.start()
...
def stop_cleaning(reason=cleaning_lib.DONE_STOPPED):
